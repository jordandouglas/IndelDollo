...
import phoneme.RegularSoundChangeModelPrior;
import phoneme.RegularSoundChangeModelPrior.ConcertEvent;
import phoneme.RegularSoundChangeSubstitutionModel;
...
public class PrunedTreeLikelihoodMutationCounter extends PrunedTreeLikelihood {
	...
	// counters for keeping track of number of mutations
    private int totalMutationCount, regularMutationCount, sporadicMutationCount;
...
//        }
        ...
        if (!m_useAmbiguities.get()) {
        	...
        	throw new IllegalArgumentException("ambiguities not implemented yet");
        }
//        
//        
...
//    }
    ...
    public void redrawAncestralStates() {
        ...
        Node root = prunedTree.getCognateMRCA();
    	
        ((PrunedTreeLikelihoodCore)this.likelihoodCore).setRootNr(root.getNr());
        
        totalMutationCount = 0;
        regularMutationCount = 0;
        sporadicMutationCount = 0;
        traverseSample(tree, root, null);
...
//
            	...
            	if (beagle != null || beaglePruned != null) {
            		throw new IllegalArgumentException("don't know how to handle BEAGLE");
...
//                    }
                ...
                countMutations(node, parentState, reconstructedStates[nodeNum]);
            }
...
//        	}
    ...
    private void countMutations(Node node, int[] parentState, int[] state) {
    	RegularSoundChangeSubstitutionModel rssm = (RegularSoundChangeSubstitutionModel) substitutionModel;
    	RegularSoundChangeModelPrior model = rssm.modelInput.get();
    	List<ConcertEvent> events = model.getSortedConcertEventsOnBranch(node);
    	
    	int [] currentState = new int[parentState.length];
    	System.arraycopy(parentState, 0, currentState, 0, parentState.length);
    			
    	
		for (ConcertEvent event : events) {
			int from = event.getFrom();
			int to = event.getTo();
			for (int i = 0; i < state.length; i++) {
				if (currentState[i] == from) {
					regularMutationCount++;
					totalMutationCount++;
					currentState[i] = to;
				}
			}
    ...
    public void init(PrintStream out) {
    	out.print("total.Mutations\t");
    	out.print("regular.Mutations\t");  // regular = concerted event
    	out.print("sporadic.Mutations\t"); // irregular = sporadic event
    }
    ...
    public void log(final long sample, final PrintStream out) {
        ...
        out.print(totalMutationCount + "\t");
        out.print(regularMutationCount + "\t");
        out.print(sporadicMutationCount + "\t");
    }
