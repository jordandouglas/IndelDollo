...
public class PrunedBeagleTreeLikelihood extends TreeLikelihood {
...
//    private double [] storedCurrentCategoryRates;
	...
	private CognatePrunedTree prunedTree;
	
	private Node originNode, birthNode, dummyLeafNode;
    ...
    public void initAndValidate() {
        ...
        originNode = new Node();
        originNode.setNr(m_nNodeCount - 1);
        originNode.setID("originNode");
        
        dummyLeafNode = new Node();
        dummyLeafNode.setNr(m_nNodeCount/2);

        originNode.setLeft(dummyLeafNode);
        dummyLeafNode.setParent(originNode);
        dummyLeafNode.setHeight(0.0);
        dummyLeafNode.setID("dummyLeafNode");

        Node root = prunedTree.getRoot();
        originNode.setRight(root);
        originNode.setHeight(root.getHeight());
    }

    private boolean initialize() {
        m_nNodeCount = treeInput.get().getNodeCount() + 2;
        ...
        tipCount = treeInput.get().getLeafNodeCount() + 1;
...
//                preferenceFlags |= BeagleFlag.SCALING_MANUAL.getMask();
        ...
        // set dummy leaf node partials or states
        if (m_bUseAmbiguities || m_bUseTipLikelihoods) {
            double[] partials = new double[patternCount * m_nStateCount * categoryCount];
            Arrays.fill(partials, 1.0);
            beagle.setPartials(tipCount-1, partials);
        } else {
            int[] states = new int[patternCount];
            // since no ambiguities are used, all states equal or larger than statCount are
            // treated as missing data
            Arrays.fill(states, m_nStateCount);
            beagle.setTipStates(tipCount-1, states);
        }
...
//    }
    ...
    public void store() {
        ...
        if (useScaleFactors || useAutoScaling) { // Only store when actually used
            ...
            System.arraycopy(scaleBufferIndices2, 0, storedScaleBufferIndices2, 0, scaleBufferIndices2.length);
            storedScaleBufferIndicesCount2 = scaleBufferIndicesCount2;
...
//            storedRescalingCount = rescalingCount;
    ...
    public void restore() {
        ...
        if (useScaleFactors || useAutoScaling) {
            ...
            tmp2 = storedScaleBufferIndices2;
            storedScaleBufferIndices2 = scaleBufferIndices2;
            scaleBufferIndices2 = tmp2;
            scaleBufferIndicesCount2 = storedScaleBufferIndicesCount2;
...
//        updateRestrictedNodePartials = true;
    ...
    public double calculateLogP() {
        ...
        if (matrixUpdateIndices == null) {
            ...
            scaleBufferIndices2 = new int[internalNodeCount];
            storedScaleBufferIndices = new int[internalNodeCount];
            storedScaleBufferIndices2 = new int[internalNodeCount];
        }
...
//     	String newick = this.prunedTree.getTrueRoot().toNewick();
     	
//     	double bt = this.prunedTree.birthInput.get().getArrayValue();
     	
     	//final Node root = prunedTree.getBirthRoot();
     	birthNode = prunedTree.getCognateMRCA();
        originNode.setRight(birthNode);
        originNode.setHeight(prunedTree.getBirthTime());
//     	final Node trueRoot = prunedTree.getTrueRoot();
...
//         ((PrunedTreeLikelihoodCore)this.likelihoodCore).setRootNr(root.getNr());
         ...
         try {
     		...
     		if (birthNode.getHeight() > prunedTree.getBirthTime()) {
     			logP = Double.NEGATIVE_INFINITY;
     			return logP;
     		}
         	
     		scaleBufferIndicesCount2 = 0;
     		traverse(originNode, null, true);
         }
...
//        }
        ...
        do {
            ...
            int rootIndex = partialBufferHelper.getOffsetIndex(m_nNodeCount-1);
...
//                    beagle.accumulateScaleFactors(scaleBufferIndices, internalNodeCount, cumulateScaleBufferIndex);
                    beagle.accumulateScaleFactors(scaleBufferIndices2, scaleBufferIndicesCount2, cumulateScaleBufferIndex);
                } else {
                    cumulateScaleBufferIndex = scaleBufferHelper.getOffsetIndex(internalNodeCount);
                }
...
//            double [] outFactors = new double[patternCount];
//            beagle.getLogScaleFactors(cumulateScaleBufferIndex, outFactors);
//            double [] rawRootPartials = new double[patternCount * m_nStateCount];
            ...
            if (Double.isNaN(logL) || Double.isInfinite(logL)) {
                ...
                if (firstRescaleAttempt && (rescalingScheme == PartialsRescalingScheme.DYNAMIC || rescalingScheme == PartialsRescalingScheme.DELAYED)) {
                    ...
                    // scale buffer indices though as we are recomputing them.
             		scaleBufferIndicesCount2 = 0;
                    traverse(originNode, null, false);
...
//    }
    ...
    private int traverse(Node node, int[] operatorNumber, boolean flip) {
        ...
        int nodeNum = node.isLeaf() ? node.getNr() : node.getNr() + 1;
        if (node == originNode) {
        	nodeNum = m_nNodeCount - 1;
        }
...
//        }
        if (!node.isRoot() || node == birthNode) {
	        final double parentHeight = node == birthNode ? prunedTree.getBirthTime() : node.getParent().getHeight();
	        final double branchRate = node.isRoot() ? 
	        		branchRateModel.meanRateInput.get().getArrayValue() : 
	        		branchRateModel.getRateForBranch(node);
	        final double branchTime = (parentHeight - node.getHeight()) * branchRate;
	        
	        if (!(node == originNode) && (update != Tree.IS_CLEAN || branchTime != m_branchLengths[nodeNum])) {
            ...
            if (update1 != Tree.IS_CLEAN || update2 != Tree.IS_CLEAN) {
                ...
                if (useScaleFactors) {
                    ...
                    if (recomputeScaleFactors) {
                        ...
                        scaleBufferIndices2[scaleBufferIndicesCount2++] = scaleBufferIndices[n];
                    ...
                    if (useAutoScaling) {
                        ...
                        scaleBufferIndices2[scaleBufferIndicesCount2++] = scaleBufferIndices[nodeNum - tipCount];
                    }
                    operations[x + 1] = Beagle.NONE; // Not using scaleFactors
                    operations[x + 2] = Beagle.NONE;
                }

                final int child1Nr = child1.isLeaf() ? child1.getNr() : child1.getNr() + 1;
                final int child2Nr = child2.isLeaf() ? child2.getNr() : child2.getNr() + 1;
                operations[x + 3] = partialBufferHelper.getOffsetIndex(child1Nr); // source node 1
                operations[x + 4] = matrixBufferHelper.getOffsetIndex(child1Nr); // source matrix 1
                operations[x + 5] = partialBufferHelper.getOffsetIndex(child2Nr); // source node 2
                operations[x + 6] = matrixBufferHelper.getOffsetIndex(child2Nr); // source matrix 2
                ...
                if (useAutoScaling) {                	
                	scaleBufferIndices2[scaleBufferIndicesCount2++] = partialBufferHelper.getOffsetIndex(nodeNum);
                } else {
                	scaleBufferIndices2[scaleBufferIndicesCount2++] = scaleBufferHelper.getOffsetIndex(nodeNum - tipCount);
            	}                
            }
        }
    ...
    private int[] scaleBufferIndices, scaleBufferIndices2;
    private int scaleBufferIndicesCount2;
    private int[] storedScaleBufferIndices;
    private int storedScaleBufferIndicesCount2;
    private int[] storedScaleBufferIndices2;
